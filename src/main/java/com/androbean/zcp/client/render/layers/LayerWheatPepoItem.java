...
public class LayerWheatPepoItem implements LayerRenderer<EntityWheatPepo> {
    protected final RenderLivingBase<EntityWheatPepo> livingEntityRenderer;
    public LayerWheatPepoItem(RenderLivingBase<EntityWheatPepo> livingEntityRendererIn) {
        this.livingEntityRenderer = livingEntityRendererIn;
    }
    public void doRenderLayer(EntityWheatPepo entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    ...
    private void renderHeldItem(EntityWheatPepo entity, ItemStack stack, ItemCameraTransforms.TransformType camera, EnumHandSide handSide) {
        if (!stack.isEmpty()) {
            GlStateManager.pushMatrix();
