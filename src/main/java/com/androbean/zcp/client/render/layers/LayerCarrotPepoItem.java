...
public class LayerCarrotPepoItem implements LayerRenderer<EntityCarrotPepo> {
    protected final RenderLivingBase<EntityCarrotPepo> livingEntityRenderer;
    public LayerCarrotPepoItem(RenderLivingBase<EntityCarrotPepo> livingEntityRendererIn) {
        this.livingEntityRenderer = livingEntityRendererIn;
    }
    public void doRenderLayer(EntityCarrotPepo entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    ...
    private void renderHeldItem(EntityCarrotPepo entity, ItemStack stack, ItemCameraTransforms.TransformType camera, EnumHandSide handSide) {
        if (!stack.isEmpty()) {
            GlStateManager.pushMatrix();
