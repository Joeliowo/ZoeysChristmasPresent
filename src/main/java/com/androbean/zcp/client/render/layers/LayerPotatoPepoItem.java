...
public class LayerPotatoPepoItem implements LayerRenderer<EntityPotatoPepo> {
    protected final RenderLivingBase<EntityPotatoPepo> livingEntityRenderer;
    public LayerPotatoPepoItem(RenderLivingBase<EntityPotatoPepo> livingEntityRendererIn) {
        this.livingEntityRenderer = livingEntityRendererIn;
    }
    public void doRenderLayer(EntityPotatoPepo entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    ...
    private void renderHeldItem(EntityPotatoPepo entity, ItemStack stack, ItemCameraTransforms.TransformType camera, EnumHandSide handSide) {
        if (!stack.isEmpty()) {
            GlStateManager.pushMatrix();
